...
namespace DesktopIcons
{
    ...
    public class MainWindowModel
    {
        ...
        private ObservableCollection<Folder> FoldersB { get; set; } = [];
        public string url = "";
        ...
        public void Resel(string highlightedUrl)
        {
            for (int i = 0; i < FoldersB.Count; i++)
            {
                if (Folders[i].Name == highlightedUrl)
                {
                    Folders[i].Selected = "#ff0000ff";
                }
                else
                {
                    Folders[i].Selected = "#00000000";
                }
            }
        }
        
        public void Navigate(string url, string highlightedUrl = null)
        {
            Folders = [];
            this.url = url;
            ...
            if (Directory.Exists(url))
            {
                ...
                foreach (DirectoryInfo directoryInfo in folder.GetDirectories().OrderBy(f => f.Name))
                {
                    ...
                    var color = "#00000000"; 
                    if ((highlightedUrl != null) && (directoryInfo.Name == highlightedUrl))
                    {
                        color = "#ff0000ff";
                    }
                    Folder f = new() { Name = directoryInfo.Name, Type = "Kataloog", Image = GetResource("Kaust"), Selected = null};
                    Folders.Add(f);
                }
                foreach (FileInfo file in folder.GetFiles().OrderBy(f => f.Name))
                {
                    ...
                    Folder f = new() { Name = file.Name, Type = GetTypeDescription(extension), Image = GetTypeIcon(extension), Selected = null};
                    Folders.Add(f);
                }
            }
            else
